...
#define RV3028_BACKUP_BSM_LSM    (BIT(2) | BIT(3))
...
static int rv3028_ioctl(struct device *dev, unsigned int cmd, unsigned long arg)
{
	...
	case RTC_VL_READ:
		...
		status = status & RV3028_STATUS_PORF ? RV3028_STATUS_PORF : 0;
...
// commented as not supported in this old a kernel
//	if (!client->irq)
//		clear_bit(RTC_FEATURE_ALARM, rv3028->rtc->features);
    ...
    if (battery_backed)
    {
      ret = rv3028_update_cfg(rv3028, RV3028_CLKOUT, 0xff, 0x00);   /* no ouputs */
          if (ret)
                  return ret;
    }
	/* setup timestamping */
		...
		if (i < ARRAY_SIZE(rv3028_trickle_resistors)) {
			if (battery_backed)
			{
				ret = rv3028_update_cfg(rv3028, RV3028_BACKUP, RV3028_BACKUP_TCE |
						 RV3028_BACKUP_TCR_MASK | RV3028_BACKUP_BSM_MASK, RV3028_BACKUP_TCE | i | RV3028_BACKUP_BSM_LSM );
			}
			else
			{
				ret = rv3028_update_cfg(rv3028, RV3028_BACKUP, RV3028_BACKUP_TCE |
